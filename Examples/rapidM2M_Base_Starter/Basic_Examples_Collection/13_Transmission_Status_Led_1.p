 ...
 *
 * Initiates a connection to the server and uses the status LED to indicate the current connection state
 * Status LED is off 	  if the device is offline (not connected) or in "Wakeup" mode
 ...
 * Status LED blinks 	  if the connection establishment has failed
 * 
...
{  
  ...
  STATE_OFF   = 0,                          // Device is offline (not connected) or in "Wakeup" mode  (Status LED off)
  STATE_ON,                                 // Device is connected to the server                      (Status LED on)
  STATE_SLOW,                               // Connection establishment failed                        (Status LED blinking)
...
HandleTxState()
{
  ...
  if(iTxState != iTxStateAkt)                  // If the connection status has changed ->  
  {
    ...
    else                                       // Otherwise ->
    {
      ...
      {
        print("FAILED | ");
		ChangeState(STATE_SLOW);       // Status LED should blink slowly   
      }
  ...
  if(iTimerLed > 0)                            // If the counter used to control the status LED has not expired ->
  {
    ...
    if(iTimerLed == 0)                         // If the counter used to control the status LED has expired ->
    {
      switch(iState)                           // Switches current state that should be displayed via the status LED 
      {
        ...
        {
          MxLed_Off(true);
          MxLed_Blink(0);                      // Makes the status LED blink until another MxLed_xxx function is called
        }
        case STATE_FAST:
        {
          MxLed_Off(true);
