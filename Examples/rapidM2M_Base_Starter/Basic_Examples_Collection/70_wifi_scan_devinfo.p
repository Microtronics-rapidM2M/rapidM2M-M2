 ...
 *           |_|
 *
 * Extended "WiFi" Example
 * 
 * Scans for WiFi network in the receiving range every 5 seconds and issues security type, WiFi RF channel and Service Set Identifier 
 * of the found WiFi networks via the console. After the initialisation of the WiFi interface the name/designation, hardware version, 
 * firmware version and MAC adress of the built-in WiFi module are also issued once via the console. 
 *
...
public WiFi_Callback(event, connhandle, const data{}, len) 
{
  ...
  if(event == WIFI_EVENT_DEVINFO)              // If information about the built-in WiFi module is available
  {
    new sDevInfo[TWiFi_DevInfo];               // Temporary memory for information about the built-in WiFi module
    ...
    rM2M_GetPackedB(data, 0, sDevInfo.chipname, 20+1);
    rM2M_GetPackedB(data, 21, sDevInfo.hwrev, 20+1);
    rM2M_GetPackedB(data, 42, sDevInfo.fwrev, 20+1);
    rM2M_GetPackedB(data, 63, sDevInfo.mac, 6);
	
    Event_DevInfo(sDevInfo);                   // Calls up the function to process the received information about the built-in WiFi module 
  }
  else if (event == WIFI_EVENT_SCAN)           // If an access point was found during scan
  {
    /* Copies the information from the data block to the temporary memory with a specific structure.
	   This is necessary as the information in the data block is packed and therefore cannot be processed directly. */
...
/**
 * Function to process the received information about the built-in WiFi module 
 *
 * @param sDevInfo:TWiFi_DevInfo - Information about a WiFi network in the receiving range
 */
Event_DevInfo(sDevInfo[TWiFi_DevInfo])
{
  //Issues the name/designation, hardware version and firmware version of the built-in WiFi module via the console 
  printf("DEVINFO: %s,%s,%s,", sDevInfo.chipname, sDevInfo.hwrev, sDevInfo.fwrev);
  
  //Issues the MAC address of the built-in WiFi module via the console
  printf("%02X:%02X:%02X:%02X:%02X:%02X\r\n",
         sDevInfo.mac{0}, sDevInfo.mac{1}, sDevInfo.mac{2},
         sDevInfo.mac{3}, sDevInfo.mac{4}, sDevInfo.mac{5});
